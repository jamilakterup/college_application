...
if(payType==''){alert ('Please select a type'); return false;}
 ...
 $("#admission_step").click(function(){	
	...
	var group =$('.group').val();
       ...
       $.ajax({	
        type:'POST',
        url:'formfillup_admission/check',
	    ...
	    if(status==5){
		$('#next_step_error').html('<span style="color:red;">Reg. number is Wrong');  
		$('#information').hide(); 
	    }
            if(status==11)
            {
            ...
            if(status==8)
            {  
                    var url ='formfillup_admission/paytype';
                    window.location=url;
            }
            if(status==1)
            {
                    var url ='formfillup_admission/view';
                    window.location=url;
            }
        }
    });
