...
if(payType==''){alert ('Please select a type'); return false;}
 ...
 $("#admission_step").click(function(){	
 	 ...
 	 $('#admission_step_modal').modal('show'); 
 	  ...
 	  {
 	  	 $('#next_step_error').html('<span style="color:red;">Select Exam Year ');
         $('#information').hide();
 	  }
       ...
       $.ajax({	
        ...
        success:function(response){
            ...
            if(status==7 )
            {
               $('#next_step_error').html('<span style="color:red;">Not Found');
               $('#information').hide();
            ...
            if(status==6)
            {
               $('#next_step_error').html('<span style="color:red;">You are not a Masters Student.');  
               $('#information').hide(); 
            }
			
            if(status==8)
            {  
                    var url ='formfillup/paytype';
                    window.location=url;
            }			
            if(status==1)
            {
