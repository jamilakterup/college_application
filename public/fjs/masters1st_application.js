 ...
 $("#admission_step").click(function(){ 
  ...
  // var current_level = $('#current_level').val();
    if(roll =='')
    {
      ...
      if(status==3){
          $('#next_step_error').html('<span style="color:red;">Application is not opened.');
               $('#information').hide();
      }
      if(status==4){
          $('#next_step_error').html('<span style="color:red;">Application date is expired.');
               $('#information').hide();
      }
      if(status==7){
      ...
      if(status==6){
          var url ='Masters1st/dbblapplication';
          window.location=url;
      }
