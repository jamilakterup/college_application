	...
	// var current_level = $('#current_level').val();
       ...
       $.ajax({	
        ...
        success:function(response){
			...
			if(status==1){
					var url ='Honours/form';
					window.location=url;
			}

      if(status==8){
          var url ='Honours/dbblapplication';
          window.location=url;
      }
      ...
      if(status==3){
          $('#next_step_error').html('<span style="color:red;">Honours Application is not opened.');
               $('#information').hide();
      }
      if(status==7){
      ...
      if(status==4){
          $('#next_step_error').html('<span style="color:red;">Honours Application date is expired.');
               $('#information').hide();
      }
