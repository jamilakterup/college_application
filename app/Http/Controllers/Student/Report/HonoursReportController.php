...
use Mpdf\Mpdf;
use Excel;
use App\Exports\Honours\Admission\HonAdmExport;
use App\Exports\Honours\Formfillup\HonsFFExport;

class HonoursReportController extends Controller
{
    public function honours_admission(Request $request){
        ...
        $breadcrumb = 'student:Student Honours Management|Dashboard';

        $query = Study::searchHonsStudent($id, $admission_roll, $faculty,$dept_name, $current_level, $session, $from_date, $to_date);

        $total_amount = 0;

        $total_amount = $query->sum('student_info_hons.total_amount');

        $num_rows = $query->count();
        $students = $query->paginate(Study::paginate());

        return view('BackEnd.student.report.honours.admreport', compact('title', 'breadcrumb', 'students','id','admission_roll','faculty','dept_name','current_level','session','num_rows', 'from_date','to_date','total_amount'));
    ...
    public function generateHonAdmReport(Request $request){
        ...
        $id = $request->get('id');
        return $admission_roll = $request->get('admission_roll');
        $faculty = $request->get('faculty');
        $dept_name = $request->get('dept_name');
        $current_level = $request->get('current_level');
        $session = $request->get('session');
        ...
        return Excel::download(new HonAdmExport($request, $students), 'hons_admission_reports_'.$session.'.xlsx');
        ...
        if($request->get('type') =='csv'){
            ...
            $filename = 'hons_admission_reports_'.$session.'.csv';
        ...
        $mpdf->WriteHTML(view('BackEnd.student.report.pdf.honadmreport', compact('students','session')));
        $mpdf->Output();

    }

    public function honours_form_fillup(Request $request){
        ...
        return view('BackEnd.student.report.honours.honsffreport', compact('title', 'breadcrumb', 'students','registraion_id','faculty','dept_name','current_level','session','formfillup_type','exam_year','num_rows', 'from_date','to_date','total_amount'));
    }

    public function generateHonsFFReport(Request $request){
        ini_set("pcre.backtrack_limit", "5000000");
  ...
  public function generateHonAppReport(Request $request)
  {
      try {
          ...
          $validated = $this->validate([
              ...
              'admission_roll' => 'nullable|string',
          ...
          $admission_roll = $validated['admission_roll'];
          ...
          $query = DB::table('hons_student_applications')
              ->select([
                  'admission_roll',
                  'name',
                  'dept_name',
          ...
          if ($from_date) {
              $query->whereDate('date', '>=', date('Y-m-d', strtotime($from_date)));
          }

          if ($to_date) {
              $query->whereDate('date', '<=', date('Y-m-d',strtotime($to_date)));
          }
          ...
          if ($request->input('type') === 'csv') {
              $filename = 'hons_application_reports_' . now()->format('Ymd_His') . '.csv';
              ...
              $file = fopen($tempPath . '/' . $filename, 'w');
              ...
              foreach ($applications as $application) {
                  $invoice = DB::table('invoices')
                      ->select('txnid')
                      ->where('type', 'honours_application')
                      ->where('roll', $application->admission_roll)
                      ->where('status', 'Paid')
                      ->first();

                  fputcsv($file, [
                      $application->admission_roll,
                      $application->name,
                      $application->dept_name,
                      $application->session,
                      ...
                      $application->admission_form,
                      $application->hsc_transcript ? $application->admission_roll . '_' . $application->hsc_transcript : ''
                  ]);
              }
          ...
          $pdf->WriteHTML(view('BackEnd.student.report.pdf.honappreport', compact(
              'applications',
              'exam_year',
              'session'
          ))->render());

          return response($pdf->Output('honours_application_report.pdf', 'I'))
              ->header('Content-Type', 'application/pdf');

      } catch (\Exception $e) {
          // Log the error
          \Log::error('Error generating honours application report: ' . $e->getMessage());
