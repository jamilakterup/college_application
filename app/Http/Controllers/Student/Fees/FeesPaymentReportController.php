...
{
    protected $path = 'BackEnd.student.fees.';
    ...
    {
        return view($this->path.'payment.report');
    }
    ...
    private function applyFilters($query, Request $request)
    {
        ...
        if ($request->filled('reference_id')) {
            $query->whereRaw("JSON_UNQUOTE(JSON_EXTRACT(reference_data, '$.reference_id')) LIKE ?", ["%{$request->reference_id}%"]);
        }
    ...
    public function getData(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('reference_id', function ($row) {
                ...
                return $referenceData['reference_id'] ?? 'N/A';
            })
            ...
            ->filterColumn('reference_id', function ($query, $keyword) {
                $query->whereRaw("JSON_UNQUOTE(JSON_EXTRACT(reference_data, '$.reference_id')) LIKE ?", ["%$keyword%"]);
            })
    ...
    public function getDetails(Request $request)
    {
        try {
            ...
            $details = [
                ...
                'reference_id' => $referenceData['reference_id'] ?? 'N/A',
        ...
        try {
            ...
            $callback = function() use ($applications) {
                ...
                foreach ($applications as $application) {
                    ...
                    fputcsv($file, [
                        $application->id,
                        $referenceData['reference_id'] ?? 'N/A',
                        $application->name,
