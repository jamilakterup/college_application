...
class HonoursController extends Controller
{
    ...
    {
         $this->middleware('permission:student.honours.create', ['only' => ['create','store']]);
         $this->middleware('permission:student.honours.edit', ['only' => ['edit','update']]);
         $this->middleware('permission:student.honours.delete', ['only' => ['destroy']]);
    }
    ...
    public function index(Request $request)
    {
        ...
        try {
            return view('BackEnd.student.admission.honours.index');
    ...
    public function datasource(Request $request){
        $records = StudentInfoHons::query();

        return DataTables::of($records)
            ->addColumn('actions','BackEnd.student.admission.honours.particles.action_buttons')
            ...
            ->editColumn('image', function($records){
                $html = "<img style='height:50px;' src='".asset("upload/college/honours/".$records->session."/".$records->image)."' class='img-thumbnail'>";
                return $html;
            })
    ...
    {
        $html = view('BackEnd.student.admission.honours.particles.form')->render();
        ...
        try {
          ...
          DB::table('student_info_hons')->insert(
            ...
            if($logo!=''){
                ...
                DB::table('hons_admitted_student')->where('auto_id', $admitted_student->auto_id)->update([
                   'photo' => $filename
                ]);

                DB::table('student_info_hons')->where('id', $id)->update([
                    'image' => $filename
                ]);
            }
    ...
    public function edit(Request $request, $id)
    {
        ...
        try {
            $student = StudentInfoHons::find($id);
            $data = [
                'student' => $student
            ];
            $html = view('BackEnd.student.admission.honours.particles.form', $data)->render();
    ...
    public function destroy(Request $request, $id)
    {
        ...
        try {
            $student = StudentInfoHons::find($id);
            HonsAdmittedStudent::where('auto_id', $student->refference_id)->delete();
            $student->delete();
    ...
    public function force_promotion(Request $request){
        ...
        try {
            foreach ($request->ids as $id) {
                ...
                if($student->current_level=='Honours 1st Year')
                {
                    $student->current_level='Honours 2nd Year'; 
                    $year='2nd'; 
                }
                else if($student->current_level=='Honours 2nd Year')
                {
                    $student->current_level='Honours 3rd Year'; 
                    $year='3rd';                 
                }
                else
                {
                    $student->current_level='Honours 4th Year';
                    $year='4th';          
    ...
    public function regStudent(Request $request)
    {
        ...
        return view('BackEnd.student.admission.honours.regStudent', compact('title', 'id','students', 'breadcrumb', 'num_rows', 'session', 'adm_roll'));
    }
    ...
    public function regSearch(Request $request) {
        ...
        return view('BackEnd.student.admission.honours.regStudent', \compact('title', 'breadcrumb', 'students'));
    ...
    public function printDetails($id){
        ...
        $student = DB::table('student_info_hons')->where('id', $id)->first();
        $admitted_student = DB::table('hons_admitted_student')->where('auto_id', $student->refference_id)->where('session', $student->session)->first();
