...
class DegreeFormfillupController extends Controller
{
  ...
  public function check(Request $request){ 
    ...
    if($request->ajax())
    {
      ...
      $results = DB::select("SELECT * FROM student_info_degree_formfillup WHERE id=$sid");
      ...
      if(count($results) > 0){
       ...
       if($degree_level=='Degree'){
        ...
        if ($ff_result_count > 0) {
          ...
          $already_regular = Invoice::where('roll', $sid)->where('slip_type', 'LIKE','%_regular%')->where('slip_type', 'NOT LIKE','%_irregular%')->where('type','degree_form_fillup')->where('date_start', '>=', $opening_date)->orderBy('id', 'desc')->where('status', 'Pending')->get();
          ...
          $already_paid_irregular = Invoice::where('roll', $sid)->where('slip_type', 'LIKE','%_irregular%')->where('type','degree_form_fillup')->where('date_start', '>=', $opening_date)->orderBy('id', 'desc')->where('status', 'Paid')->get();
  ...
  public function view() 
    {
      ...
      $payment_amount='';
        ...
        {
            $student_infos=DB::table('student_info_degree_formfillup')->where('id',$registration_id )->get();
    ...
    public function dbblPageView() 
    {
      ...
      $examyear =   Session::get('ex_year');
        ...
        $student = DB::table('student_info_degree_formfillup')->where('id', $student_id)->first();

        $already_exists = DB::table('invoices')->where('roll', $student->id)->where('passing_year', $examyear)->where('type', 'degree_form_fillup')->where('level', $current_level)->where('date_start', '>=', $opening_date)->where('status', 'Pending')->orderBy('id', 'desc')->get();
          ...
          DB::table('invoices')->insert(
            ...
            $invoice = DB::table('invoices')->where('roll', $student->id)->where('passing_year', $examyear)->where('type', 'degree_form_fillup')->where('date_start', '>=', $opening_date)->where('level', $current_level)->where('status', 'Pending')->orderBy('id', 'desc')->first();
...
$results = DB::select("select * from payment_info where id='$payment_info_id'");
    ...
    $student_infos = DB::table('student_info_degree_formfillup')->where('id', $student_id)->get();
  ...
  public function payment_view(Request $request){
    ...
    $examyear =   Session::get('ex_year'); 
    $invoices    = DB::table('invoices')->where('type', 'degree_form_fillup')->where('level', $current_level)->where('passing_year', $examyear)->where('roll', $student_id)->where('date_start', '>=', $opening_date)->orderBy('id', 'desc')->get();
      ...
      if($request->ajax())
         {
            ...
            $student_info = DB::table('student_info_degree_formfillup')->where('id', $registration_id)->first();
