...
class DegreePrivateRegController extends Controller
{
    public function index() {
        Auth::logout();
        Session::flush();
        return view('application.degreePrivate.index');
    }
    ...
    public function checkApplication(Request $request){
        ...
        $payslipheaders = DB::table('payslipheaders')->where('type', 'registration')->where('level', 'Degree 2nd Year')->where('pro_group', 'degree')->where('session', $admission_session)->get();

        if (count($payslipheaders) < 1) {
            return $status = 2;
        }else{
            $payslipheader = $payslipheaders->first();
            $admission_name = 'degree_registration_'.$payslipheader->level.'_'.$payslipheader->exam_year;
            $total_amount = DB::table('payslipgenerators')->where('payslipheader_id', $payslipheader->id)->sum('fees');

            $total_amount = (int) $total_amount;

            $invoices = Invoice::where('roll', $admission_roll)->where('level', $current_level)->where('date_start', '>=', $config->opening_date)->where('type', 'degree_registration')->orderBy('id', 'desc')->get();
            $invoice_id = '';
            if (count($invoices) > 0) {
                $invoice = $invoices->first();
                $invoice_id = $invoice->id;

                $application_student = DB::table('degree_student_applications')->where('current_level', 'Degree 2nd Year')->where('session', $admission_session)->where('admission_roll', $admission_roll)->where('exam_year', $config->exam_year)->where('registration_type', 'Registration')->get();
                ...
                if ($invoice->status == 'Pending') {
                    ...
                    $admitted_student = DB::table('degree_application_admitted_student')->where('admission_roll',$admission_roll)->where('registration_type', 'Registration')->where('session', $admission_session)->get();
    ...
    public function dbblapplication(){
        ...
        if ($admission_roll == '') {
            return Redirect::route('student.degree.private.reg')->with('res', 'Please try again.');
        }

        $invoice = Invoice::where('roll', $admission_roll)->where('id', $invoice_id)->where('admission_session', $admission_session)->where('type', 'degree_registration')->first();
        ...
        $admitted_students = DB::table('degree_application_admitted_student')->where('session', $admission_session)->where('registration_type', 'Registration')->where('admission_roll',$admission_roll)->get();

        if (count($admitted_students) < 1) {
            return view('application.degreePrivate.form', compact('admission_roll', 'payment_status','total_amount', 'invoice_id'));
        }

        $student_applications = DB::table('degree_student_applications')->where('current_level',$current_level)->where('session', $admission_session)->where('admission_roll', $admission_roll)->where('registration_type', 'Registration')->get();
        ...
        return view('application.degreePrivate.dbblapplication', compact('admission_roll', 'payment_status','total_amount'));
    }
    ...
    public function applicationForm(){
        ...
        if ($admission_roll != ''  || $invoice_id == '') {
            return view('application.degreePrivate.form', compact('admission_roll', 'invoice_id'));
        }

        return redirect()->route('student.degree.private.reg');
    }
    ...
    public function degAppInformationSubmit(Request $request){
        ...
        $config = DB::table('admission_config')->where('course', 'degree')->where('open', 1)->where('current_level', 'Degree 2nd Year')->where('type', 'registration')->first();

        $admission_session = $config->session;

        $invoices = Invoice::where('roll', $admission_roll)->where('id', $invoice_id)->where('admission_session', $admission_session)->where('type', 'degree_registration')->get();
        ...
        $admitted_student = DB::table('degree_application_admitted_student')->where('session', $admission_session)->where('registration_type', 'Registration')->where('admission_roll',$admission_roll)->get();
        ...
        DB::table('payment_info')
          ...
          DB::table('invoices')->where('type', 'degree_registration')->where('id', $invoice->id)->update(['name'=>$request->get('student_name'),'subject'=> $request->get('dept_name')]);

          $total_amount = $toatalPayAmount;
          $payment_status = 'Pending';

        return redirect()->route('student.degree.private.reg.dbbl');
    }
    ...
    public function confirmslip(){
        ...
        $invoice = Invoice::where('roll', $admission_roll)->where('id', $invoice_id)->where('admission_session', $admission_session)->where('type', 'degree_registration')->first();
        $student = DB::table('degree_student_applications')->where('current_level', 'Degree 2nd Year')->where('session', $admission_session)->where('admission_roll', $admission_roll)->where('registration_type', 'Registration')->first();
        ...
        $html = view('application.degreePrivate.slip_id', compact('student', 'invoice', 'tracking_id'));
