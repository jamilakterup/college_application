...
class HonoursApplicationController extends Controller
{
    public function index() {
        Auth::logout();
        Session::flush();
        return view('application.honours.index');
    }
    ...
    public function checkApplication(Request $request){
        ...
        $configs = DB::table('admission_config')->where('course', 'honours')->where('type', 'application')->where('open', 1)->where('current_level', 'Honours 1st Year')->get();
        ...
        $payslipheaders = DB::table('payslipheaders')->where('type', 'application')->where('level', 'Honours 1st Year')->where('pro_group', 'honours')->where('session', $admission_session)->get();

        if (count($payslipheaders) < 1) {
            return $status = 2;
        }else{
            $payslipheader = $payslipheaders->first();
            $admission_name = 'honours_application_'.$payslipheader->level.'_'.$payslipheader->exam_year;
            $total_amount = DB::table('payslipgenerators')->where('payslipheader_id', $payslipheader->id)->sum('fees');

            $total_amount = (int) $total_amount;

            $invoices = Invoice::where('roll', $admission_roll)->where('level', $current_level)->where('date_start', '>=', $config->opening_date)->where('type', 'honours_application')->orderBy('id', 'desc')->get();
            $invoice_id = '';
            if (count($invoices) > 0) {
                ...
                $admitted_students = DB::table('hons_application_admitted_student')->where('session', $admission_session)->where('admission_roll',$admission_roll)->get();

                $student_applications = DB::table('hons_student_applications')->where('current_level', 'Honours 1st Year')->where('session', $admission_session)->where('admission_roll', $admission_roll)->get();

                if(count($admitted_students) > 0 && $invoice->status == 'Paid') return $status = 8;

                if(count($student_applications) < 1 ||  $invoice->status == 'Pending'){
                    return $status = 1;
                }


                if(count($admitted_students) > 0 && count($student_applications) > 0){
                    return $status = 8;
                }

                $application_student = DB::table('hons_student_applications')->where('current_level', 'Honours 1st Year')->where('session', $admission_session)->where('admission_roll', $admission_roll)->where('exam_year', $config->exam_year)->get();
                ...
                if ($invoice->status == 'Pending') {
                    ...
                    $admitted_student = DB::table('hons_application_admitted_student')->where('admission_roll',$admission_roll)->where('session', $admission_session)->get();
                    if (count($admitted_student) > 0) {
                        $auto= auto_id_msc($admitted_student[0]->auto_id);
                        Session::put('tracking_id', HONS_PREF.$auto);
                    }
                }
            }else{
    ...
    public function dbblapplication(){
        ...
        if ($admission_roll == '') {
            return Redirect::route('honours.student.application')->with('res', 'Please try again.');
        }

        $invoice = Invoice::where('roll', $admission_roll)->where('id', $invoice_id)->where('admission_session', $admission_session)->where('type', 'honours_application')->first();
        ...
        if (count($student_applications) < 1) {
            return view('application.honours.dbblapplication', compact('admission_roll', 'payment_status','total_amount', 'invoice_id'));
        }
        ...
        if (count($admitted_students) > 0) {
            ...
            Session::put('tracking_id',HONS_PREF.$admitted_student->auto_id);
        }


        return view('application.honours.dbblapplication', compact('admission_roll', 'payment_status','total_amount'));
    }
    ...
    public function applicationForm(){
        ...
        if ($admission_roll != ''  || $invoice_id == '') {
            return view('application.honours.form', compact('admission_roll','invoice_id'));
        }
    }

    public function honAppInformationSubmit(Request $request){
        $this->validate($request, [
            'student_name' => 'required',
            'father_name' => 'required',
            'contact_no' => 'required|numeric',
            'admission_form' => 'required|file|mimes:jpg,jpeg,png|max:1024',
            'hsc_transcript' => 'required|file|mimes:jpg,jpeg,png|max:1024'
        ]);
        ...
        $config = DB::table('admission_config')->where('course', 'honours')->where('open', 1)->where('current_level', 'Honours 1st Year')->where('type', 'application')->where('clossing_date', '>=', date('Y-m-d'))->first();

        $admission_session = $config->session;

        $invoices = Invoice::where('roll', $admission_roll)->where('id', $invoice_id)->where('admission_session', $admission_session)->where('type', 'honours_application')->get();
        ...
        if($request->hasFile('admission_form')){
            $admission_form = $request->file('admission_form');
              $form_filename = rand(1, 99999999999) .'.jpg';
              $upload_path = public_path('upload/college/honours/application/admission_form/' . $form_filename);
              Image::make($admission_form->getRealPath())->save($upload_path);
        }

        if($request->hasFile('hsc_transcript')){
            $hsc_transcript = $request->file('hsc_transcript');
              $transcript_filename = rand(1, 99999999999) .'.jpg';
              $upload_path = public_path('upload/college/honours/application/hsc_transcript/' . $transcript_filename);
              Image::make($hsc_transcript->getRealPath())->save($upload_path);
        }
        ...
        $submitted_data = array(
                ...
                'admission_form'=> $form_filename ?? null ,
                'hsc_transcript'=> $transcript_filename ?? null,
                'application_invoice_id'=>$invoice->id
          );

        $admitted_student = DB::table('hons_application_admitted_student')->where('session', $admission_session)->where('admission_roll',$admission_roll)->get();

        if(count($admitted_student) > 0){
            DB::table('hons_application_admitted_student')->where('auto_id', $admitted_student->first()->auto_id)
              ->update($submitted_data);
            $admitted_id = $admitted_student->first()->auto_id;
        }else{
            $admitted_id = DB::table('hons_application_admitted_student')
              ->insertGetId($submitted_data);
        }

        $admitted_student = DB::table('hons_application_admitted_student')->where('auto_id', $admitted_id)->where('session', $admission_session)->where('admission_roll',$admission_roll)->get();
        ...
        DB::table('payment_info')
          ...
          DB::table('invoices')->where('type', 'honours_application')->where('id', $invoice->id)->update(['name'=>$request->get('student_name')]);

          $total_amount = $toatalPayAmount;
          $payment_status = 'Pending';

        return redirect()->route('honours.student.application.dbbl');
    }
    ...
    public function confirmslip(){
        ...
        $invoice = Invoice::where('roll', $admission_roll)->where('id', $invoice_id)->where('admission_session', $admission_session)->where('type', 'honours_application')->first();
        $student = DB::table('hons_student_applications')->where('current_level', 'Honours 1st Year')->where('session', $admission_session)->where('admission_roll', $admission_roll)->first();
        ...
        $html = view('application.honours.slip_id', compact('student', 'invoice', 'tracking_id'));
    ...
    public function downloadHonsForm(){
        ...
        $configs = DB::table('admission_config')->where('course', 'honours')->where('open', 1)->where('current_level', 'Honours 1st Year')->where('type', 'application')->where('clossing_date', '>=', date('Y-m-d'))->get();
        ...
        $admitted_student = DB::table('hons_application_admitted_student')->where('admission_roll', $admission_roll)->where('session', $config->session)->where('auto_id', $auto_id)->get();
        ...
        $html = view('application.honours.form_id', compact('admitted_student'));
    ...
    public function mscSignin(){
      return view('application.honours.sign_in_form');
    }          
  ...
  public function retrievepass(Request $request){
    ...
    $results = DB::table('hons_application_admitted_student')->where('admission_roll',$admission_roll )->where('hsc_roll',$hsc_roll)->get();
  ...
  public function mscStudentSignin(Request $request){
      ...
      $config = DB::table('admission_config')->where('course', 'honours')->where('current_level', 'Honours 1st Year')->where('clossing_date', '>=', date('Y-m-d'))->where('open', 1)->get();
        if(count($config) > 0){
      ...
      if($auto_id = msc_tracking_auto_id($tracking_id)){
        $admitted_students = DB::table('hons_application_admitted_student')->where('session', $conf->session)->where('auto_id',$auto_id)->where('password',$password)->get();
      ...
      if($number>0){
        ...
        if ($admitted_student->admission_invoice_id == 0) {
            $invoices = Invoice::where('roll', $admitted_student->admission_roll)->where('level', 'Honours 1st Year')->where('date_start', '>=',$conf->opening_date)->where('date_end','<=' ,$conf->clossing_date)->where('admission_session', $conf->session)->where('type', 'honours_application')->get();
              if(count($invoices) > 0){
                $invoice = $invoices->first();
    ...
    public function applicationLogout(){
       Auth::logout(); 
      Session::flush();
      return Redirect::route('honours.student.application'); 
    }
}
