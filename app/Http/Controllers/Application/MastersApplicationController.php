...
class MastersApplicationController extends Controller
{
    public function index() {
        Auth::logout();
        Session::flush();
        return view('application.masters.index');
    }
    ...
    public function checkApplication(Request $request){
        ...
        $configs = DB::table('admission_config')->where('course', 'masters')->where('type', 'application')->where('open', 1)->where('current_level', 'Masters 2nd Year')->get();
        ...
        $payslipheaders = DB::table('payslipheaders')->where('type', 'application')->where('level', 'Masters 2nd Year')->where('pro_group', 'masters')->where('session', $admission_session)->get();
        ...
        if (count($payslipheaders) < 1) {
            ...
            $invoices = Invoice::where('roll', $admission_roll)->where('level', $current_level)->where('date_start', '>=', $config->opening_date)->where('date_end', '<=', $config->clossing_date)->where('type', 'masters_2nd_application')->orderBy('id', 'desc')->get();
            $invoice_id = '';
            if (count($invoices) > 0) {
                $invoice = $invoices->first();
                $invoice_id = $invoice->id;

                $application_student = DB::table('masters_student_applications')->where('current_level', 'Masters 2nd Year')->where('session', $admission_session)->where('admission_roll', $admission_roll)->where('exam_year', $config->exam_year)->get();
                ...
                if ($invoice->status == 'Pending') {
                    ...
                    $admitted_student = DB::table('masters_application_admitted_student')->where('admission_roll',$admission_roll)->where('application_invoice_id', $invoice_id)->where('session', $admission_session)->get();
    ...
    public function dbblapplication(){
        ...
        if ($admission_roll == '') {
            return Redirect::route('masters.student.application')->with('res', 'Please try again.');
        }

        $invoice = Invoice::where('roll', $admission_roll)->where('id', $invoice_id)->where('admission_session', $admission_session)->where('type', 'masters_2nd_application')->first();
        ...
        if (count($admitted_students) < 1) {
            return view('application.masters.form', compact('admission_roll', 'payment_status','total_amount', 'invoice_id'));
        }
        ...
        if (count($student_applications) < 1) {
            return view('application.masters.dbblapplication', compact('admission_roll', 'payment_status','total_amount', 'invoice_id'));
        }
        ...
        return view('application.masters.dbblapplication', compact('admission_roll', 'payment_status','total_amount'));
    }
    ...
    public function applicationForm(){
        ...
        if ($admission_roll != ''  || $invoice_id == '') {
            return view('application.masters.form', compact('admission_roll', 'invoice_id'));
        }

        return redirect()->route('masters.student.application');
    }
    ...
    public function mscAppInformationSubmit(Request $request){
        ...
        $config = DB::table('admission_config')->where('course', 'masters')->where('open', 1)->where('current_level', 'Masters 2nd Year')->first();

        $admission_session = $config->session;

        $invoices = Invoice::where('roll', $admission_roll)->where('id', $invoice_id)->where('admission_session', $admission_session)->where('type', 'masters_2nd_application')->get();
        ...
        DB::table('payment_info')
          ...
          DB::table('invoices')->where('type', 'masters_2nd_application')->where('id', $invoice->id)->update(['name'=>$request->get('student_name'),'subject'=> $request->get('dept_name')]);

          $total_amount = $toatalPayAmount;
          $payment_status = 'Pending';

        return redirect()->route('masters.student.application.dbbl');
    }
    ...
    public function confirmslip(){
        ...
        $invoice = Invoice::where('roll', $admission_roll)->where('id', $invoice_id)->where('admission_session', $admission_session)->where('type', 'masters_2nd_application')->first();
        $student = DB::table('masters_student_applications')->where('current_level', 'Masters 2nd Year')->where('session', $admission_session)->where('admission_roll', $admission_roll)->first();
        ...
        $html = view('application.masters.slip_id', compact('student', 'invoice', 'tracking_id'));
    ...
    public function downloadHonsForm(){
        ...
        $configs = DB::table('admission_config')->where('course', 'masters')->where('open', 1)->where('current_level', 'Masters 2nd Year')->get();
        ...
        $html = view('application.masters.form_id', compact('admitted_student'));
    ...
    public function mscSignin(){
      return view('application.masters.sign_in_form');
    }          
  ...
  public function mscStudentSignin(Request $request){
      ...
      $config = DB::table('admission_config')->where('course', 'masters')->where('current_level', 'Masters 2nd Year')->where('clossing_date', '>=', date('Y-m-d'))->where('open', 1)->get();
        if(count($config) > 0){
      ...
      if($number>0){
        ...
        if ($admitted_student->admission_invoice_id == 0) {
            $invoices = Invoice::where('roll', $admitted_student->admission_roll)->where('level', 'Masters 2nd Year')->where('date_start', '>=',$conf->opening_date)->where('date_end','<=' ,$conf->clossing_date)->where('admission_session', $conf->session)->where('type', 'masters_2nd_application')->get();
              if(count($invoices) > 0){
                $invoice = $invoices->first();
