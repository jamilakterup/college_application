...
class MastersFormfillupController extends Controller
{
    public function index() { 

    Session::flush();
    return view('mastersformfillup.index');
  ...
  public function check(Request $request){
    ...
    if($request->ajax())
    {
      ...
      if(count($results) > 0){
        ...
        $configs = DB::table('form_fillup_config')->where('current_level',$current_level)->where('open', 1)->where('clossing_date', '>=', date('Y-m-d'))->where('course', 'masters')->get();
        if (count($configs) > 0) {
  ...
  public function view() 
    {
      ...
      $payment_amount='';
        ...
        if($admission_step<1)
        {
          return Redirect::route('masters.student.formfillup');
        }
        else
        {
            $student_infos=DB::table('student_info_masters_formfillup')->where('current_level', $current_level)->where('id',$registration_id )->get();
           return view('mastersformfillup.view', compact('registration_id', 'student_infos', 'payment_amount', 'payment_status', 'prc', 'imp', 'admission_step', 'student_id'));
        }
    ...
    public function dbblPageView() 
    {
      ...
      $examyear =   Session::get('ex_year');
        ...
        if (count($already_paid) > 0) {
          return redirect()->route('masters.student.formfillup.view');
        }
        ...
        $already_exists = DB::table('invoices')->where('roll', $student->id)->where('passing_year', $examyear)->where('type', 'masters_form_fillup')->where('level', $current_level)->where('date_start', '>=', $opening_date)->where('admission_session', $session)->where('status', 'Pending')->orderBy('id', 'desc')->get();
          ...
          $invoice_id = DB::table('invoices')->insertGetId(
            ...
            $invoice = DB::table('invoices')->where('roll', $student->id)->where('passing_year', $examyear)->where('type', 'masters_form_fillup')->where('date_start', '>=', $opening_date)->where('level', $current_level)->where('admission_session', $session)->where('status', 'Pending')->orderBy('id', 'desc')->first();
    ...
    $student_infos = DB::table('student_info_masters_formfillup')->where('current_level', $current_level)->where('id', $student_id)->get();
        ...
        return view('mastersformfillup.dbbl_view', compact('payment_amount','student_infos', 'student_id', 'institute_code' ,'current_level'));
    }
  ...
  public function payment_view(Request $request){
    ...
    $session = Session::get('session');
    $examyear =   Session::get('ex_year'); 
    $invoices    = DB::table('invoices')->where('type', 'masters_form_fillup')->where('level', $current_level)->where('passing_year', $examyear)->where('roll', $student_id)->where('date_start', '>=', $opening_date)->where('admission_session', $session)->orderBy('id', 'desc')->get();
    ...
    return view('mastersformfillup.dbbl_view', compact('payment_amount', 'student_id', 'student_infos'));
  }
      ...
      if($request->ajax())
         {
            ...
            $current_level =  Session::get('current_level');
            $degree_config = DB::table('form_fillup_config')->where('current_level',$current_level)->where('open', 1)->where('course', 'masters')->first();
            $ff_level = $degree_config->current_level;

            $student_info = DB::table('student_info_masters_formfillup')->where('current_level', $current_level)->where('id', $registration_id)->first();
            ...
            $html = view('mastersformfillup.slip_id', compact('ff_student'));
  ...
  public function nextStep(){
    ...
    return view('mastersformfillup.paymenttype', compact('student_id', 'current_level', 'faculty_name', 'subject', 'session', 'examyear'))->withStudent_id($student_id)->withCurrent_level($current_level)->withFaculty_name($faculty_name);
  }
