...
<div class="panel">
    <header class="panel-heading">
      <h3 class="panel-title">{{ucfirst($course)}} Admission Configs</h3>
    </header>
    <div class="panel-body">
      <table class="d-flex justify-content-center w-full">
       <tr>
         ...
         <td>
           {!! Form::select('type', getEnumValues('admission_config', 'type'), null , ['class'=> 'form-control form-control-sm selectize filter', 'id' => 'type', 'data-placeholder' => '<--Select Type-->']) !!}
         </td>
...
$(document).ready(function() {
  datatable = $('#datatable').DataTable({
      ajax:{
        url: "{{ route('admin.admission.config.datasource') }}",
        data: function (d) {
          d._token = "{{ csrf_token() }}",
          d.course = $('#course').val()
          d.open = $('#open').val(),
