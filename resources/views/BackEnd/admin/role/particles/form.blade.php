    		...
    		@foreach ($permission_parent_groups as $parent_group)
	    		...
	    		@foreach ($permission_groups as $group)
	    			...
	    			<tr>
	    				...
	    				@endif
		    			<td>
		    				<input type="checkbox" class="" id="{{ $i }}Management" value="{{ $group->name }}" onclick="checkPermissionByGroup('role-{{ $i }}-management-checkbox', this)">
									...
									<tbody>
										<tr>
											<td style="border: none; border-right:1px solid #ddd; border-left:none !important; width:190px;">
												<input type="checkbox" class="module" value="{{ $group }}" onclick="checkPermissionByModule('role-{{ $i }}-management-checkbox', this)">
												{{$group}}
											</td>
												...
												@foreach ($modules as $item)
													...
													@foreach ($permissions as $permission)
														@if($permission->name == $permission_name)
															...
															<input type="checkbox" onclick="checkSinglePermission('role-{{ $i }}-management-checkbox', '{{ $i }}Management', {{ count($permissions) }})" name="permissions[]" id="checkPermission{{ $permission->id }}" value="{{ $permission->name }}">
