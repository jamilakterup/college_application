...
function num_eng_to_ben($english) {
	...
	if($status=='active') 
	{
		echo $name." is not released still now.";
		exit();
	}

    $results=DB::select('select * from designation WHERE id="'.$position.'"');
    $position_name=null;
        ...
        <table width='100%' class='sign'>
            ...
            <tr>
                <td style='padding-left:25px' align='right'>
                    <br/><br/><br/>({{$name}})<br/><span style='font-size:10px'>(আইডি নং-{{$id_ben}})</span><br/>মুক্ত অফিসার
                </td>		
            </tr>
            ...
            <tr>
                ...
                <td align='right' style='padding-right:20px'>
                    তারিখ : {{num_eng_to_ben(date('d/m/Y',strtotime($release_date)))}}
                </td>		
            </tr>
            ...
            <tr>
                <td align='left' colspan='2'>
                    ...
                    ০৪। অধ্যক্ষ, {{$outgoing_college}} । (বিদায়ী)<br/>				
                    ০৫। জনাব {{$name}} ({{$id_ben}}), {{$department_ben}} বিভাগ, {{$college}}, {{$area}}। (বিমুক্ত)<br/>
                    ০৬। জনাব _________________________________________ {{$college}}, {{$area}}। (গ্রহণকারী)<br/>				
