@php
    ...
    $department = DB::table('student_info_hons')->whereIn('id',$student_ids)->where('session', $request->session)->groupBy('dept_name')->get();
...
<table border="1">
	...
	<thead>
		...
		<tr>
			<td style="text-align: center; color: #C00000; font-size: 15px; font-weight: bold;" colspan="{{$head_colspan}}">{{$request->current_level ?? 'Honours'}} Admission {{$request->session}}</td>
		</tr>
		<tr>
	...
	<tbody border="1">
		...
		@foreach ($departments as $key =>  $dept)
			@php
				$dept_students = DB::table('student_info_hons')->leftJoin('admission_students','student_info_hons.id','=','admission_students.id')->whereIn('student_info_hons.id', $student_ids)->where('student_info_hons.session', $request->session)->where('student_info_hons.dept_name', $dept)->select('student_info_hons.id','admission_students.total_amount','admission_students.transaction_id','admission_students.date','student_info_hons.merit_status')->get();
