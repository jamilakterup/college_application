...
<table border="1">
	...
	<thead>
		...
		<tr>
			<td style="text-align: center; background-color: #FFFF00; font-size: 15px; font-weight: bold;" colspan="{{$head_colspan}}">Indivisual Statement of Preliminary to Masters {{$request->exam_year}}</td>
		</tr>
	...
	<tbody border="1">
		<tr>
			...
			<td colspan="3" style="background-color: #B7DEE8; border: 1px solid #ddd; font-weight: bold;">Department Name</td>		
		...
		@foreach ($departments as $key =>  $dept)
			@php
				$dept_students = DB::table('form_fillup')->whereIn('id', $student_ids)->where('level_study', $request->current_level)->where('exam_year', $request->exam_year)->where('dept_name', $dept->dept_name)->get();
			...
			<tr>
				...
				<td colspan="3" style="border: 1px solid #ddd; background-color: #FCE4D6;">{{$dept->dept_name}}</td>
