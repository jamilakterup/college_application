	...
	<tbody border="1">
		...
		@foreach($slip_groups as $slip){
			<tr>
				...
				@foreach ($departments as $dept)
					@php
						$dept_students = DB::table('form_fillup')->whereIn('id', $student_ids)->where('level_study', $request->current_level)->where('exam_year', $request->exam_year)->where('groups', $dept)->where('total_amount', $slip->total_amount)->where('slip_name', $slip->slip_name)->get();
