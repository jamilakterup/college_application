@php
	...
	$slip_groups = DB::table('form_fillup')->whereIn('id', $student_ids)->where('level_study', $request->current_level)->where('exam_year', $request->exam_year)->where('groups', $request->groups)->groupBy('total_amount','slip_name')->orderBy('total_amount', 'asc')->get();
...
<table border="1">
	...
	<thead>
		...
		<tr>
			<td style="text-align: center; background-color: #FFFF00; font-size: 15px; font-weight: bold;" colspan="{{$head_colspan}}">Statement of {{$request->groups}}</td>
		</tr>
		...
		@foreach($slip_groups as $slip){
			<tr>
				...
				@php
					...
					$group_students = DB::table('form_fillup')->whereIn('id', $student_ids)->where('level_study', $request->current_level)->where('exam_year', $request->exam_year)->where('groups', $request->groups)->where('total_amount', $slip->total_amount)->where('slip_name', $slip->slip_name)->get();
