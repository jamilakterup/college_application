...
<title>Masters Formfillup Student Lists, Session-{{$session}}</title>
</head>
<body>
<style>
.page-break {
...
<h2 style="text-align: center;">Masters Formfillup Student Lists {{$session !='' ? 'Session: '.$session:''}}</h2>
...
<?php $si=0; ?>
	<?php foreach($records as $application) {?>
				...
				<tr class="">
					...
					<td style="font-size:12px;"><?php echo $application->dept_name ?></td>
