...
<title>HSC 2nd Year Admission Student Lists, Session-{{$session}}</title>
</head>
<body>
<style>
.page-break {
...
<h2 style="text-align: center;">HSC 2nd Year Admission Student Lists {{$session !='' ? 'Session: '.$session:''}}</h2>
...
<?php $si=0; ?>
	<?php foreach($records as $application) {?>
				<?php $si++; ?>
				<tr class="">
				    <td style="font-size:12px;"><?php echo $si ?></td>
					<td style="font-size:12px;"><?php echo $application->id ?></td>
					<td style="font-size:12px;"><?php echo $application->name ?></td>	
					<td style="font-size:12px;"><?php echo $application->groups ?></td>
