...
<title>Honours Admission Student Lists, Session-{{$session}}</title>
</head>
<body>
<style>
.page-break {
...
<h2 style="text-align: center;">Honours Admission Student Lists {{$session !='' ? 'Session: '.$session:''}}</h2>
...
<table class="order-details" border="1" width="100%" style="margin-top:10px;">
	...
	<tr>
		...
		<th style="border:1px solid #cecece;">Department Name</th>
...
<?php $si=0; ?>
	<?php foreach($students as $adm) {?>
				@php
					$admitted_student = DB::table('hons_admitted_student')->where('auto_id', $adm->refference_id)->first();
				@endphp
				<tr class="">
				    <td style="font-size:12px;"><?php echo $adm->id ?></td>
					<td style="font-size:12px;"><?php echo $adm->admission_roll ?></td>
					<td style="font-size:12px;"><?php echo $adm->name ?></td>
					<td style="font-size:12px;"><?php echo $adm->dept_name ?></td>	
					<td style="font-size:12px;"><?php echo $adm->contact_no ?></td>
					<td style="font-size:12px;"><?php echo $adm->total_amount ?></td>
					<td style="font-size:12px;"><?php echo $adm->date ?></td>
				</tr>	
