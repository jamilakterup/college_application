...
<head>
	...
	<title>Masters Admission Student Lists, Session-{{$session}}</title>
</head>
<body>
	<style>
		.page-break {
	...
	<h2 style="text-align: center;">Masters Admission Student Lists {{$session !='' ? 'Session: '.$session:''}}</h2>
	...
	<table class="order-details" border="1" width="100%" style="margin-top:10px;">
		...
		<?php foreach($students as $adm) {?>
			@php
			$admitted_student = DB::table('masters_admitted_student')->where('auto_id', $adm->refference_id)->get();
			@endphp
			<tr class="">
				<td style="font-size:12px;"><?php echo $adm->id ?></td>
				<td style="font-size:12px;">{{$adm->admission_roll}}</td>
