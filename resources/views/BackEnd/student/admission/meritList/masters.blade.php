...
@section('page-title', 'Masters Merit List Management')
...
<div class="panel">
        <header class="panel-heading">
          <div class="panel-actions">
            <a href="{{ route('student.meritlist.upload', ['type'=> 'masters']) }}" class="btn btn-sm btn-info" data-label="Masters Merit Student"><i class="fas fa-upload"></i> Upload Batch Merit List</a>
            <a href="{{ route('student.meritlist.create', ['type'=> 'masters']) }}" class="btn btn-sm btn-primary add_new" data-label="Masters Merit Student"><i class="fal fa-plus"></i> Add New Merit Student</a>
          </div>
          <h3 class="panel-title">Masters Merit Lists</h3>
        </header>
        <div class="panel-body">
          ...
          <form class="form-inline d-flex justify-content-center">
            ...
            <div class="form-group">
              {!! Form::select('current_level', filter_empty_array(selective_multiple_masters_level()) ,null , ['class'=> 'form-control form-control-sm selectize filter', 'id' => 'current_level', 'data-placeholder' => '<--Current Level-->']) !!}
            </div>
...
$(document).ready(function() {
  datatable = $('#datatable').DataTable({
      ajax:{
        url: "{{ route('student.meritlist.masters.datasource') }}",
        data: function (d) {
          d._token = "{{ csrf_token() }}",
