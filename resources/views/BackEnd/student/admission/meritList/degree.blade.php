...
@section('page-title', 'Degree Merit List Management')
...
<div class="panel">
        <header class="panel-heading">
          <div class="panel-actions">
            <a href="{{ route('student.meritlist.upload', ['type'=> 'degree']) }}" class="btn btn-sm btn-info" data-label="Degree Merit Student"><i class="fas fa-upload"></i> Upload Batch Merit List</a>
            <a href="{{ route('student.meritlist.create', ['type'=> 'degree']) }}" class="btn btn-sm btn-primary add_new" data-label="Degree Merit Student"><i class="fal fa-plus"></i> Add New Merit Student</a>
          </div>
          <h3 class="panel-title">Degree Merit Lists</h3>
        </header>
        <div class="panel-body">
          ...
          <form class="form-inline d-flex justify-content-center">
            ...
            <div class="form-group">
              {!! Form::select('groups', filter_empty_array(selective_degree_subjects()) ,null , ['class'=> 'form-control form-control-sm selectize filter', 'id' => 'groups', 'data-placeholder' => '<--Groups-->']) !!}
            </div>
...
$(document).ready(function() {
  datatable = $('#datatable').DataTable({
      ajax:{
        url: "{{ route('student.meritlist.degree.datasource') }}",
        data: function (d) {
          d._token = "{{ csrf_token() }}",
