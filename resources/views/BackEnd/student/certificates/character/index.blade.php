...
@section('title', 'Character Certificate Management')
        ...
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">
                    <i class="fa fa-certificate"></i> Character Certificate Management
                    <div class="pull-right">
                        <a href="{{ route('certificates.character.upload') }}" class="btn btn-info btn-sm">
                            <i class="fa fa-upload"></i> Bulk Upload
                        </a>
                        <a href="{{ route('certificates.character.create') }}" class="btn btn-success btn-sm">
            ...
            <div class="panel-body">
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered" id="characterTable">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @forelse($certificates as $key => $certificate)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('certificates.character.show', $certificate->id) }}"
                                                class="btn btn-info btn-sm" title="View">
                                                <i class="fa fa-eye"></i>
                                            </a>
                                            <a href="{{ route('certificates.character.pdf', $certificate->id) }}"
                                            ...
                                            <form action="{{ route('certificates.character.destroy', $certificate->id) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="11" class="text-center">No character certificates found</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
...
@push('scripts')
    <script>
        ...
        function bulkDownloadPdf() {
            ...
            var form = $('#bulkForm');
            form.attr('action', '{{ route('certificates.character.bulk-pdf') }}');
        ...
        function bulkStatusUpdate(status) {
            ...
            if (confirm('Are you sure you want to update the status of selected certificates?')) {
                var form = $('#bulkForm');
                form.attr('action', '{{ route('certificates.character.bulk-status') }}');
                form.find('input[name="ids[]"]').remove();
        ...
        function bulkDelete() {
            ...
            if (confirm('Are you sure you want to delete the selected certificates? This action cannot be undone.')) {
                var form = $('#bulkForm');
                form.attr('action', '{{ route('certificates.character.bulk-delete') }}');
                form.find('input[name="ids[]"]').remove();
