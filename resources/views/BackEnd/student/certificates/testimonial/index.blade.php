    ...
    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title">
                <i class="fa fa-certificate"></i> Testimonial Management
                <div class="pull-right">
                    <a href="{{ route('certificates.testimonial.upload') }}" class="btn btn-info btn-sm">
                        <i class="fa fa-upload"></i> Bulk Upload
                    </a>
                    <a href="{{ route('certificates.testimonial.create') }}" class="btn btn-success btn-sm">
                ...
                <table class="table table-striped table-bordered" id="testimonialTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group">
                                    <a href="{{ route('certificates.testimonial.show', $testimonial->id) }}" 
                                    ...
                                    <form action="{{ route('certificates.testimonial.destroy', $testimonial->id) }}" 
...
function bulkDownloadPdf() {
    ...
    var form = $('#bulkForm');
    form.attr('action', '{{ route("certificates.testimonial.bulk-pdf") }}');
...
function bulkStatusUpdate(status) {
    ...
    if (confirm('Are you sure you want to update the status of selected testimonials?')) {
        var form = $('#bulkForm');
        form.attr('action', '{{ route("certificates.testimonial.bulk-status") }}');
        form.find('input[name="ids[]"]').remove();
...
function bulkDelete() {
    ...
    if (confirm('Are you sure you want to delete the selected testimonials? This action cannot be undone.')) {
        var form = $('#bulkForm');
        form.attr('action', '{{ route("certificates.testimonial.bulk-delete") }}');
        form.find('input[name="ids[]"]').remove();
