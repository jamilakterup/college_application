...
<body>
    ...
    <p style="font-weight: bold;font-size: 16px;text-transform: uppercase; text-align: center; margin-bottom: 0;">
        {{ config('settings.college_name') }}
    ...
    @php
        ...
        foreach ($summary as $group => $val) {
            ...
            $groupTotalStu = StudentInfoHsc::where('groups', $group)->where('session', $session)->count() ?? 0;
        ...
        foreach ($scales as $letter) {
            foreach (array_keys($summary) as $group) {
                $totalLetterStudent = count(
                    array_filter($data[$group] ?? [], fn($student) => $student['grade'] == $letter),
                );
