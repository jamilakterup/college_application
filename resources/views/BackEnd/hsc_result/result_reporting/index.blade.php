    ...
    <div class="panel">
        ...
        <div class="panel-body">
            ...
            <div class="col-md-12 d-flex justify-content-center mb-5">
                {!! Form::open([
                    'route' => 'hsc_result.result_reporting_subject_wise',
                    'method' => 'post',
                    'class' => 'form-inline',
                ]) !!}
                <div class="form-group">
                    {!! Form::select('groups', create_option_array('groups', 'id', 'name', 'Group'), $groups, [
                        'class' => 'form-control',
                    ]) !!}
                </div>

                <div class="form-group">
                    {!! Form::select('exams', create_option_array('exams', 'id', 'name', 'Exam'), $exams, [
                        'class' => 'form-control',
                    ]) !!}
                </div>
                ...
                <div class="form-group">
                    {!! Form::select('exam_year', selective_multiple_exam_year(), $exam_year ?? null, [
                        'class' => 'form-control exam_year',
                        'autocomplete' => 'off',
                    ]) !!}
                </div>
                ...
                @if (count($resultgp) > 0)
                    <button type="submit" name="export" value="1" class="btn btn-success ml-1">Export to
                        Excel</button>
                @endif
                {!! Form::close() !!}
            </div>
    ...
    <div class="panel">
        ...
        <div class="panel-body">
            <div class="col-md-12 d-flex justify-content-center">
                ...
                <div class="form-group">
                    {!! Form::select('subjects', create_option_array('subjects', 'id', 'name', 'Subject'), $subjects, [
                        'class' => 'form-control',
                    ]) !!}
                </div>
                ...
                <div class="form-group">
                    <select name="grade_scales" class="form-control input-sm">
                        ...
                        <option value="{{ $result->point }}" @if ($result->point == $grade_scales) {{ 'selected' }} @endif>
                            {{ $result->letter_grade }}</option>
                        <?php  } ?>
                        <option value="all">All</option>
                        <option value="p" @if ('p' == $grade_scales) {{ 'selected' }} @endif>pass</option>
                    </select>
                </div>

                <button type="submit" class="btn btn-info">Search</button>
                @if (!empty($filter) && $filter == 'all')
                    <button type="submit" name="print" value="2" class="btn btn-success ml-1">Export to PDF</button>
                @endif
                @if (count($resultgp) > 0)
                    <button type="submit" name="export" value="1" class="btn btn-success ml-1">Export to
                        Excel</button>
