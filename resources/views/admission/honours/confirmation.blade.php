...
$admission_roll = Session::get('admission_roll');
  ...
  $class_roll = '';
  $college_id = '';
  $results = DB::table('hons_admitted_student')->where('session', $admission_session)->where('admission_roll', $admission_roll)->where('auto_id',$auto_id)->get();
...
$invoices = DB::table('invoices')->where('id', $invoice_id)->where('admission_session', $session)->where('roll', $admission_roll)->where('type', 'honours_admission')->get();
  ...
  if(count($invoices) > 0){
    ...
    $results = DB::select("SELECT class_roll,id FROM student_info_hons WHERE refference_id=$auto_id 
    and session='$session' and  current_level='Honours 1st Year'");
...
?>
  ...
  <div class="container-fluid">
    ...
    <a href="<?php echo url('/').'/Admission/Honours/logout'?>" class="btn btn-danger navbar-btn">Logout</a>
  </div>
</nav>
    ...
    <div class="row mb">
          ...
          <div class="panel-body">
            @if($payment_status!="Paid")
                @php
                  ...
                  $payment_guideline = get_config('honours_adm_payment_guideline');
            ...
            @endif
              ...
              <a id="honconfirm_slip" target="_blank" style="cursor: pointer;">Download Confirmation Slip</a>
              <br/> <br/>
      ...
      <div class="col-md-4" style="float: right;">
        <div class="panel panel-info">
          ...
          <div class="panel-body">
              <img  class="user_pic_view pull-center img-polaroid"  src="<?php echo url('/').'/upload/college/honours/draft/'. $photo;?>" alt="User Photo" /> 
...
<script src="<?php  echo url('/') ?>/fjs/honours_admission.js"></script>
