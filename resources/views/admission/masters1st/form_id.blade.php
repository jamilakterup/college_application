...
<head>
    ...
    <title>Masters Part-1 Admission Form</title>
    {{-- <link rel="stylesheet" type="text/css" href="{{ asset('css/masters_admission_form.css') }}"> --}}
    <style>
    ...
    <header class="clearfix">
        ...
        <div class="header-content clearfix">
            <div class="left">
                ...
                <h4>Masters Part-1 Admission Form</h4>
        ...
        <h4  style="margin-bottom: 2px; margin-top: 11px;">Academic Information: </h4>
            <table class="left-info">
                ...
                <tr>
                    ...
                    <td>{{$adm_student->groups}}</td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">Admitted Department</td>
                    <td>{{$adm_student->dept_name}}</td>
                </tr>
                @if($adm_student->is_migrated == 1)
                    @php
                    $migrations = DB::table('migrated_student')->where('admission_roll', $adm_student->admission_roll)->where('course', 'masters')->where('current_level', $adm_student->current_level)->where('session', $adm_student->session)->get();
                    @endphp
                    @if(count($migrations))
                        <tr>
                            <td style="font-weight: bold;">Migrated Department</td>
                            <td>{{$migrations[0]->present_subject}}</td>
                        </tr>
                    @endif
                @endif
                <tr>
