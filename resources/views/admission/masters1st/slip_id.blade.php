...
<head>
    ...
    <title>Masters Part-1 Admission Confirmation Slip</title>
...
<body>
    <div class="slip">
        ...
        <section class="main-content">
            <h2 style="margin: 2px 0;">Masters Part-1 Admission Confirmation Slip</h2>
            <table>
                @php
                $migrations = DB::table('migrated_student')->where('admission_roll', $student->admission_roll)->where('course', 'masters')->where('current_level', $student->current_level)->where('session', $student->session)->get();
                @endphp
                ...
                @if(count($migrations))
                    <tr>
                        <td class="info-label">Migrated Department</td>
                ...
                @if(count($migrations))
                    @if($migrations[0]->payment_status != 'paid')
                        <tr>
                            <td class="info-label">Payment Amount</td>
                            <td>{{$migrations[0]->payment_diff}} | ({{$migrations[0]->payment_status}})</td>
                        </tr>
                    @endif
                @endif
