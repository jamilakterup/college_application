...
<body>
    <div>
            ...
            <div class="col-sm-10 center-block" style="float:none;">
                <div class="panel panel-primary">
                    <div class="panel-heading text-center">
                        <h2>New Gov Degree College, Rajshahi</h2>
                        <h3>Admit Card</h3>
                    </div>
                </div>
            </div>
        ...
        <!-- Panel Body Same Width as Header (col-sm-10) -->
        <div class="row">
            <div class="col-sm-10 center-block" style="float:none;">
                <div class="panel panel-default">
                    <div class="panel-heading small-heading">
                        Please provide the following information to download admit card
                    </div>
                    ...
                    <div class="panel-body">
                        ...
                        @if (!$show_transcript)
                            ...
                            {{-- SEARCH FORM --}}
                            <form action="{{ route('hsc_result.search') }}" method="POST" role="form"
                                    ...
                                    <table class="table table-bordered">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-center">GPA (Without 4th)</th>
                                                <th class="text-center">CGPA</th>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            {{-- ORIGINAL LOOP KEPT SAME --}}
                                            @php
                                                $k = 1;
                                            @endphp
                                            ...
                                            @foreach ($sub_gpa as $sub)
                                                ...
                                                <tr>
                                                    <td class="text-center">{{ $sub->subject->code }}</td>
                                                    <td>{{ $sub->subject->name }}</td>

                                                    <td class="text-center">
                                                        {{ $particle_mark[0]->converted_mark ?? '-' }}</td>

                                                    @if (count($particle_mark) == 2)
                                                        <td class="text-center">{{ $particle_mark[1]->converted_mark }}
                                                        </td>
                                                        <td class="text-center">-</td>
                                                    @elseif(count($particle_mark) == 3)
                                                        <td class="text-center">{{ $particle_mark[1]->converted_mark }}
                                                        </td>
                                                        <td class="text-center">{{ $particle_mark[2]->converted_mark }}
                                                        </td>
                                                    @else
                                                        <td class="text-center">-</td>
                                                        <td class="text-center">-</td>
                                                    ...
                                                    <td class="text-center">
                                                        {{ $sub->absent ? 'Absent' : $sub->total_mark }}
                                                    </td>

                                                    <td class="text-center">{{ $sub->grade }}</td>
                                                    <td class="text-center">{{ $sub->point }}</td>

                                                    @if ($k == 1)
                                                        <td class="text-center" rowspan="{{ $sub_gpa->count() }}">
                                                            {{ $cgpa_tot[0]->without_4th ?? '' }}
                                                        </td>

                                                        <td class="text-center"
                                                            rowspan="{{ max(1, $sub_gpa->count() - 1) }}">
                                                            {{ $cgpa_tot[0]->cgpa ?? '' }}
                                                        </td>
                                                    @endif
                                                </tr>
		                ...
		                <th>PR</th> --}}
                                ...
                                @foreach ($sub_gpa as $sub)
                                    <tr>
                                        ...
                                        @if (count($particle_mark) == 2)
                                            <td style="text-align: center;">{{ $particle_mark[1]->converted_mark }}
                                            </td>
                                            <td style="text-align: center;">-</td>
                                        @elseif(count($particle_mark) == 3)
                                            <td style="text-align: center;">{{ $particle_mark[1]->converted_mark }}
                                            </td>
                                            <td style="text-align: center;">{{ $particle_mark[2]->converted_mark }}
                                            </td>
                                        @else
                                            <td style="text-align: center;">-</td>
                                            <td style="text-align: center;">-</td>
                                        ...
                                        @if ($k == 1)
                                            <td style="text-align: center;" rowspan="{{ $sub_gpa->count() }}">
                                                {{ $cgpa_tot[0]->without_4th }}</td>
                                            <td style="text-align: center;" rowspan="{{ $sub_gpa->count() - 1 }}">
                                                {{ $cgpa_tot[0]->cgpa }}</td>
                                        @endif
                                        @if ($k == $sub_gpa->count())
                                            <td>
