        ...
        <input type="text" id="student_id" name ="student_id" class="form-control" readonly value="<?php echo $student_id; ?>">
         ...
         <div class="form-group">
            @php
                ...
                $student = DB::table('student_info_hons_formfillup')->where('id', $student_id)->where('current_level', $current_level)->orderBy('auto_id', 'desc')->first();
                
                $query = DB::table('payslipheaders')->where('pro_group', 'honours')->where('level', $current_level)->where('type', 'formfillup')->where('exam_year', $examyear);

                if($student->student_type != '') $query->where('student_type', $student->student_type);

                $query->where(function($q) use ($faculty_name,$student, $subject){

                    if ($faculty_name != '') {
                        $q->where(function($queryFac) use ($faculty_name) {
                            $queryFac->where('group_dept', 'LIKE', '%' . $faculty_name . '%');
                ...
                $headers = $query->orderBy('id', 'asc')->orderBy('total_papers', 'asc')->get();
          ...
          <select  id="payType" required="required" name="payType" class="form-control preferenceSelect">
            ...
            @foreach($headers as $paySlip)
                @php
                    ...
                    if($paySlip->group_dept != 0){
                        if(!in_array($student->faculty_name , filter_empty_array(explode('_',$paySlip->group_dept)))){
                            continue;
                        }
                    }
                    ...
                    if($paySlip->formfillup_type != 'others'){
                        ...
                        if($student->registration_type=='irregular' && $paySlip->formfillup_type == 'regular')
                            // continue regular
                            continue;
                    }
