...
<body>
  ...
  @php
    ...
    $class_roll = '';
    $college_id = '';
    $results = DB::table('hons_application_admitted_student')->where('admission_roll', $admission_roll)->where('session', $current_admission_session)->where('auto_id',$auto_id)->get();
    ...
    foreach($results as $result){
      ...
      $invoices = DB::table('invoices')->where('id', $invoice_id)->where('admission_session', $session)->where('roll', $admission_roll)->where('type', 'honours_application')->orderBy('id', 'desc')->get();
      ...
      $student_info = DB::table('hons_student_applications')->where('refference_id', $auto_id)->where('current_level', 'Honours 1st Year')->where('session', $current_admission_session)->get();
  ...
  <div class="container-fluid">
    ...
    <a href="<?php echo url('/').'/Application/Honours/logout'?>" class="btn btn-danger navbar-btn">Logout</a>
  </div>
</nav>
        ...
        <div class="panel-heading"> {{$payment_status!="Paid"? 'DBBL Payment Method':'Please Download Your Confirmation Slip'}}</div>   
          ...
          @if($payment_status!="Paid")
            ...
            @php
              ...
              $payment_guideline = get_config('honours_application_payment_guideline');

              if($payment_guideline){
                echo @configTempleteToBody($payment_guideline,['student_id'=> $student_id, 'college_name_bn'=> $college_name_bn, 'biller_id'=> $biller_id,'total_amount'=>round($total_amount)]);
              }
          @endphp
...
<script src="{{ asset('fjs/honours_application.js') }}"></script>
