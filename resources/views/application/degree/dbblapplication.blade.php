...
<body>
  ...
  @php
    ...
    foreach($results as $result){
      ...
      $invoices = DB::table('invoices')->where('id', $invoice_id)->where('admission_session', $session)->where('roll', $admission_roll)->where('type', 'degree_application')->get();
      ...
      $student_info = DB::table('degree_student_applications')->where('refference_id', $auto_id)->where('current_level', 'Degree 1st Year')->where('session', $current_admission_session)->get();
  ...
  <div class="container-fluid">
    <div class="navbar-header">
      <a class="navbar-brand" href="#">{{config('settings.college_name')}} Degree Online Application</a>
    </div>
    <a href="<?php echo url('/').'/Application/Degree' ?>" class="btn btn-danger navbar-btn">Application</a>
    <a href="<?php echo url('/').'/Application/Degree/logout'?>" class="btn btn-danger navbar-btn">Logout</a>
  </div>
</nav>
        ...
        <div class="panel-heading"> {{$payment_status!="Paid"? 'DBBL Payment Method':'Please Download Your Confirmation Slip'}}</div>
           ...
           @if($payment_status!="Paid")
              ...
              @php
                  ...
                  $payment_guideline = get_config('degree_application_payment_guideline');

                  if($payment_guideline){
                    echo @configTempleteToBody($payment_guideline,['student_id'=> $admission_roll, 'college_name_bn'=> $college_name_bn, 'biller_id'=> $biller_id,'total_amount'=>$total_amount]);
                  }
              @endphp
...
<script src="{{ asset('fjs/degree_application.js') }}"></script>
