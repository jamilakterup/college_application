...
<html>
<head>
    <title>Online Honours 2nd Year Admission</title>
...
<body>
  ...
  @php
    ...
    foreach($results as $result){
      ...
      $invoices = DB::table('invoices')->where('id', $invoice_id)->where('admission_session', $session)->where('roll', $admission_roll)->where('type', 'masters_2nd_application')->get();
      ...
      $student_info = DB::table('masters_student_applications')->where('refference_id', $auto_id)->where('current_level', 'Masters 1st Year')->where('session', $current_admission_session)->get();
  ...
  <div class="container-fluid">
    <div class="navbar-header">
      <a class="navbar-brand" href="#">{{config('settings.college_name')}} Online Honours 2nd Year Admission</a>
    </div>
    <a href="<?php echo url('/').'/Application/Masters1st' ?>" class="btn btn-danger navbar-btn">Admission</a>
    <a href="<?php echo url('/').'/Application/Masters1st/logout'?>" class="btn btn-danger navbar-btn">Logout</a>
  </div>
</nav>
        ...
        <div class="panel-heading"> {{$payment_status!="Paid"? 'DBBL Payment Method':'Please Download Your Confirmation Slip'}}</div>   
            <div class="panel-body">
              ...
              @php
                  ...
                  $payment_guideline = get_config('honours2nd_adm_payment_guideline');
