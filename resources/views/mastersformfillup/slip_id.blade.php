...
<head>
    ...
    <title>{{$ff_student->level_study}} FormFillup Confirmation Slip</title>
...
<body>
    <div class="slip">
        ...
        <section class="main-content">
            <h2 style="margin: 2px 0;">{{$ff_student->level_study}} Form Fillup Confirmation Slip</h2>
            <table>
                <tr>
                    <td class="info-label">Name</td>
                    <td>{{$ff_student->name}}</td>
                </tr>
                ...
                <tr>
                    <td class="info-label">Form Fillup Level</td>
                    <td>{{$ff_student->level_study}}</td>
                </tr>

                <tr>
                    <td class="info-label">Department Name</td>
                    <td>{{$ff_student->dept_name}}</td>
                </tr>
                @if($ff_student->pay_type != 'others')
                    <tr>
                        <td class="info-label">Registration Type</td>
                        <td>{{ucfirst($ff_student->formfillup_type)}}</td>
                    </tr>
                @else
                    <tr>
                        <td class="info-label">Payment Type</td>
                        <td>{{$ff_student->slip_name}}</td>
                    </tr>
                @endif

                @if($ff_student->total_papers != '0')
                    <tr>
                        <td class="info-label">Total Papers</td>
                        <td>{{$ff_student->total_papers}}</td>
                    </tr>
                @endif

                <tr>
                    <td class="info-label">Exam Year</td>
                    <td>{{$ff_student->exam_year}}</td>
                </tr>

                <tr>
                    <td class="info-label">Transaction ID</td>
                    <td>{{$ff_student->transaction_id}}</td>
                </tr>

                <tr>
                    <td class="info-label">Paid Amount</td>
                    <td>{{$ff_student->total_amount}}</td>
                </tr>

                <tr>
                    <td class="info-label">Payment Date</td>
                    <td>{{$ff_student->date}}</td>
                </tr>
            </table>
        </section>
        ...
        <section class="footer clearfix">
            ...
            <h3 class="company">Powered by -rajIT Solutions Ltd.</h3>
        </section>
